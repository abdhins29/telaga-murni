				...
				<table class="table table-bordered">
					...
					<tbody>
						<?php 
							...
							$no = 1;
							$qq = mysqli_query($konek, "SELECT * FROM tbl_transaksi a LEFT JOIN tbl_pelanggan b ON a.kd_pelanggan=b.kd_pelanggan LEFT JOIN tbl_mobil c ON a.kd_mobil=c.kd_mobil WHERE b.id= '$_SESSION[id]'");
			...
			<div class="modal-body">
				<form action="" method="POST" enctype="multipart/form-data">
					...
					<div class="form-group">
						...
						<select id="kd_mobil" name="kd_mobil" class="form-control mb-2">
							<option value="">Pilih Mobil</option>
							<?php
							$daftar_mobil = array();
							include ("../config/koneksi.php"); 
							$qqq = mysqli_query($konek,"SELECT * FROM tbl_mobil a LEFT JOIN tbl_jenis b ON a.kd_jenis=b.kd_jenis");
							while($ddd = mysqli_fetch_assoc($qqq)){
							if ($ddd['status'] == "1" )
								{
								$daftar_mobil[] = $ddd;
							?>
								<option value="<?php echo $ddd['kd_mobil']; ?>"><?php echo $ddd['type_mobil']; ?> - <?php echo $ddd['no_polisi']; ?></option>
							<?php 
								}
							}
							?>
...
mysqli_query($konek,"UPDATE tbl_pelanggan set status_peminjaman='$hasil_status2' where kd_pelanggan='$kd_pelanggan'");
	...
	if($save) {
      echo "<script language=javascript>
          ...
          window.location='datarental.php';
          </script>";
      }else{
        echo "<script language=javascript>
